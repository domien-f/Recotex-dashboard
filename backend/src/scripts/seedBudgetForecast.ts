...
async function main() {
  // NOTE: this seed previously deleted non-lead categories on every run, which
  // would wipe the full 2026 budget (Marketing team, IT, Sponsoring, etc.) on
  // every Coolify redeploy. That cleanup is removed — the importer
  // (importBudgetMarketing2026.ts) is now the source of truth for budget data
  // and this seed only acts as a fallback for fresh databases.

  // Only seed if table is empty — never overwrite imported / manual data
